...
#' @param method either 'tdist', 'ntrial', 'jags', \sQuote{boot} or \sQuote{im}
...
cv_pred_int <- function(formula, data, 
                          method = c("tdist", "ntrial", "jags", "boot", "im"), 
  ...
  for(i in 1:iter){
    ...
    for(j in 1:n.trials){
      ...
      if(method == "im"){
        fit <- lmer(y ~ 1 + (1 | trial), data = xm1)
        pdi <- pred_int_im(fit, level = level,...)
      }
